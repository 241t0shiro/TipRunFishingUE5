...
関連: [全体正本と要決定事項](GAME_DESIGN.md)、[実装規約](../AGENTS.md)。**本納品は設計のみ。下記タスクは未着手。** 工数・発売日・担当者は未決。
...
2. 対象リポジトリが `TipRunFishingUE5` であることを確認する。本納品フォルダをUEプロジェクトと誤認しない。
...
|---|---|---|---|
| 01 M00 | `.uproject`、C++ Runtimeモジュール、空のGameMode/Controller、基本ディレクトリを作成 | 環境確認 | UE 5.8.2でEditorターゲットビルド、空マップ起動 |
